 ...
 * @version $Id: USB.java,v 1.2 2005/01/17 07:17:40 westerma Exp $
 */
...
// FIXME:  root can change over suspend/resume cycle ...
    ...
    throws IOException
    {
        ...
        listeners = l;
        host = h;
        root=0;  //since we're flattening the hierarchy for MacOSX, each device is its own bus and root hub, with dev address assumed 1
        if(MacOSX.trace)
            System.err.println(" in usb.macosx.USB() \n");
    ...
    {
        StringBuffer buf = new StringBuffer ("{ MacOSX Bus: ");
    ...
    void kill ()
    {
        ...
        //if (listeners.size () > 0) {
            synchronized (devices) {
		        if (MacOSX.trace)
        		    System.err.println ("killing bus: " + this);
    ...
    private void removed (DeviceImpl dev)
    {
        ...
        //in Mac OS X we want to close device (blocking further ctrl msgs) before
        //      calling 'removed' listeners!  Do that in other OS's too?
    ...
    {
        synchronized (devices) {
            DeviceImpl d = (DeviceImpl)getDevice (dev.getAddress ());
            int     i;
            //if (MacOSX.trace)
              // System.err.println ("notify removeDev " + dev.getPath());

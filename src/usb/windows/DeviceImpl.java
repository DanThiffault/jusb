...
import usb.util.LangCode;
    ...
    // NOTE:  originally based on usb.linux.DeviceImpl
    // trying to have "diff" still make sense for a while
 ...
 * @version $Id: DeviceImpl.java,v 1.2 2005/01/17 07:21:42 westerma Exp $
 */
public
final class DeviceImpl extends Device implements DeviceSPI
{
    ...
    // XXX Need some device lock to safeguard
    ...
    {
	StringBuffer	buf = new StringBuffer ("{Windows Device: ");
...
/**/
     ...
     * supported by this device.  The OS assigned one already.
     *
     * @deprecated
	...
	*/
    ...
    // usbcore limitation; USB limit is 64KB
	...
	if (osname != null && osname.indexOf ("XP") != -1) {
	    return true;
	} else {
	    return false;
	}
	...
	private static native int cyclePort(int fd);
    ...
    //private static native int readBulk (int fd, int ep, byte buf [], int off, int length);
	...
	*/
    ...
    //private static native int writeBulk (int fd, int ep,byte buf [], int off, int length);
    ...
    throws USBException
    {
	    throw new USBException ("writeBulk (unimplemented)", 0);
	/*
	int result = 0;
	...
	*/
    ...
    //private static native int readIntr (int fd, int ep, byte buf [], int off, int length);
	...
	*/
    ...
    //private static native int writeIntr (int fd, int ep,byte buf [], int off, int length);
	...
	*/
    ...
    // we're done using this interface
	...
	*/
    ...
    {
	if (printed++ == 0)
	    System.err.println ("no bus model scan/update");

	/*
	byte	data [] = new byte [128];

	synchronized (lock) {
	    int	status = -1;//getHubPorts (fd, data);
		...
		if (devnum != 0) {
		    ...
		    if (Windows.debug && children [i] != null
			&& children [i].hub != this)
